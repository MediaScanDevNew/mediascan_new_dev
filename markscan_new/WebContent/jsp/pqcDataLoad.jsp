	...
	$(document).ready(function() {
		$(".one").click(function() {
			...
			$.getJSON('ajaxActionInv', {
				invlink : apple, reasn:'PMQC: '+person
				...
				alert("Nothing is Selected for PMQC")
			}
		});
	});
</script>
...
<body>
	<div>
		<form action="updatePMQCtable" name="form1" method="post">
			...
			<%-- 	<s:textfield name="date" value="%{date}"></s:textfield> --%>



			<%-- <s:hidden name="date" value="%{date}"></s:hidden>
			<s:hidden name="project" value="%{project}"></s:hidden> --%>
				...
				<s:iterator value="qcData">
					<tr>
						...
						<s:if test="%{greylist==1}">
							<td class="b" style="background-color: orange">
							<s:a
									href="isValid.jsp?id=%{id}"
									onclick="return popup(this, 'notes')">
									<s:property value="link" />
								</s:a> 
								<%-- <s:property value="greylist"/> --%>
								</td>
								<td class="c" style="background-color: orange"><s:property value="domainName" /></td>
						</s:if>
						<s:else>
