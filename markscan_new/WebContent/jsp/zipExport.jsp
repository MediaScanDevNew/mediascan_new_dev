...
<script type="text/javascript">
	...
	$(document).ready(function() { // for Domain
		$('#datepicker').change(function(event) {
			//alert(document.getElementById('datepicker').value );
			// var x = document.getElementById("datepicker").val();
			var country = document.getElementById('datepicker').value; // country = define value of selected option
			alert(country);
			//var ptypee = $("select#projecttype").val();
			//alert(ptype);
			$.getJSON('ajaxAction_date', {
				ctype : country,
			//	ptype : ptypee
			}, function(jsonResponse) {
				$('#ajaxResponse').text(jsonResponse.dummyMsg);
				var select = $('#domain');
				select.find('option').remove();
				$.each(jsonResponse.stateMape, function(key, value) {
					$('<option>').val(key).text(value).appendTo(select);
				});
			});
		});
...
<body>
	...
	<div style="margin-left: 170px;">
		...
		<s:form method="post" action="zipdataExtracted">
			...
			<s:select id="domain" name="domain" list="#{'0':'Select Domain'}"
				label="Domain" cssStyle="width: 170px;" />
	...
	<div>
		<s:if test="%{downloadfile_name=='na'}">
		   
		</s:if>
		<s:elseif test="%{downloadfile_name=='pj_error'}">
		zip file not created....
	</s:elseif>
		<s:else>
			<a href="<s:property value='downloadfile_name'  />" target="_blank">Download Zip
				file</a>
		</s:else>
