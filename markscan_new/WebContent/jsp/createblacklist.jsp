...
<div class="row"></div>
  ...
  <span aria-hidden="true">&times;</span>
        ...
        <div class="col-lg-12">      
	<s:form name="myform" action="addblacklist" cssClass="mar-top-6 font-14" theme="simple" onsubmit="return validateform()" method="post">
            ...
            <label for="p_type" class="col-sm-1 col-md-1 control-label">Blacklist Name:</label>        
        <div class="col-sm-2 col-md-2">
    ...
    <div class="modal-dialog modal-lg">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
			<s:form  action="updateblacklist" cssClass="mar-top-6 font-14" theme="simple"  method="post">
			  <input type="hidden" id="wlid" name="id" >
			  <input id="keyphrasename" type="text" name="domainname"> 
		    ...
		    success: function(data, textStatus, jqXHR)
		    {
		        ...
		        if(value.length>0){
		        	for(var i=0;i<value.length;i++){
		        		...
		        		result += '<td class="keyphrasevalue'+value[i].id+'">'+value[i].domain+'</td>';
		        		
		        		if(value[i].is_active==0){
		        			result += '<td><span class="active'+value[i].id+'"> <input type="submit" class="btn btn-primary activeinactive"   onclick="updateactive('+value[i].is_active+','+value[i].id+')"  name ="active" value="Active"></span></td>';
		        		}else if(value[i].is_active==1){
		        			result += '<td><span class="active'+value[i].id+'"> <input type="submit" class="btn btn-danger activeinactive"   onclick="updateactive('+value[i].is_active+','+value[i].id+')" name ="inactive" value="Inactive"></span></td>';
		        		}else{
		        			result += '<td><span class="active'+value[i].id+'"> <input type="submit" class="btn btn-primary" name ="undefine" value="Undefine"></span></td>';
		        		}
		        		
		        		result += '<td onclick="openmodal('+value[i].id+',\''+ value[i].domain + '\')"><input type="submit" class="btn btn-primary" name ="edit" value="Edit"</td>';
		        		 result += "</tr>";
		        	}
		        }
		       
		        $(".keyphrasedata").html(result); 
		        $('#example').DataTable();
		    },
...
function  updateactive(aval,id){
	...
	$.ajax({
	    url : "updateactiveblacklist",
	    type: "POST",
	    data : {id:id,aivalue:aval},
	    success: function(data, textStatus, jqXHR)
	    {
	    	var result = '';
