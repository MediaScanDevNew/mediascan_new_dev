...
public interface Master_crawle_urlRepository extends CrudRepository<Master_crawle_url, String> {
	...
	@Query(value = "insert into master_crawle_url (crawle_url2,project_id,user_id, stored_project_setup_id,"
			+ " pipe_id,machine, domain_name,title) VALUES (:lnk,:prid,:uid,:sid,:ppid,:machine,:domain,:title)", nativeQuery = true)
	@Transactional
	public Integer saveData(@Param("lnk") String lnk, @Param("prid") int prid, @Param("uid") int uid,
			...
			@Param("domain") String domain,@Param("title") String title);
	...
	@Modifying
	@Query(nativeQuery = true ,value="UPDATE master_crawle_url set g_list=1 where id =?1")
	@Transactional
	int updateGreylistedDomain(int id);
